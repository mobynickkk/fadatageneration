...
class FunctionGraphicService(MathServiceInterface):
    ...
    def run(self, dto: TaskDto) -> CompletedTaskDto:
        ...
        main_x = arange(dto.functions[0].range_from, dto.functions[0].range_to,dto.functions[0].step if dto.functions[0].step else step_h(dto.functions[0].range_from, dto.functions[0].range_to))
        
        if dto.function[0].use_emissions:
            main_y = []
            d = []
            for ind, el in enumerate(main_x):
                try:
                    y = eval(dto.funtions[0].function.replace('x',str(el)))
        ...
        for func in dto.functions[1:]:
            ...
            for i in arange(func.range_from, func.range_to, func.step if func.step else step_h(func.range_from, func.range_to)):
